...
Solver::Solver(uint64_t w, uint64_t h, vector<uint64_t> **x, vector<uint64_t> **y) : width(w), height(h), xAxisClue(x), yAxisClue(y) {
    for (uint64_t i = 0; i < width; ++i) {
        uint64_t *temp = new uint64_t[STAT_SIZE];
        for (uint64_t j = 0; j < STAT_SIZE; ++j) {
            temp[j] = 0;
        }
 ...
 */
void Solver::updateYStats(uint64_t column) {
    uint64_t mask = (uint64_t)1 << (width - column - 1);
    uint64_t row = (uint64_t)1 << (height - 1);
    for (uint64_t i = 0; i < height; ++i) {
...
void Solver::updateXStats(uint64_t row) {
    uint64_t mask = (uint64_t)1 << (height - row - 1);
    uint64_t column = (uint64_t)1 << (width - 1);
    for (uint64_t i = 0; i < width; ++i) {
...
bool Solver::solve() {
    ...
    */
    uint64_t x_mask = ((uint64_t)1 << height) - 1;
    uint64_t y_mask = ((uint64_t)1 << width) - 1;
    uint64_t result_solid, result_dot;
    while (true) {
