...
class Solver {
 private:
    ...
    */
    vector<uint64_t*> xStats;
    vector<uint64_t*> yStats;
    void updateYStats(uint64_t column);
    void updateXStats(uint64_t row);

 public:
    Solver(uint64_t w, uint64_t h, vector<uint64_t> **x, vector<uint64_t> **y);
    ~Solver();
    void printResult();
    bool solve();
};
